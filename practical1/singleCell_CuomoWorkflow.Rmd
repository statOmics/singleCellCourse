...
The data can be downloaded from [Zenodo](https://zenodo.org/record/3625024#.YWfahtlBxB1).
At the bottom of this web-page, we can download the files `raw_counts.csv.zip` 
and `cell_metadata_cols.tsv` and store these files locally.
...
library(data.table)
data <- fread(cmd = "unzip -p /Users/jg/Desktop/PhD/DTU_trajectory/Data/iPSC/iPSC_gene/raw_counts.csv.zip")
data[1:5,1:5] # data table with first column the gene names
```

Then we read the metadata of the experiment:

```{r}
metadata <- read.csv("/Users/jg/Desktop/PhD/DTU_trajectory/Data/iPSC/cell_metadata_cols.tsv",
                     sep="\t")
head(metadata,n=3)
```
...
As stated in the paper, cells were harvested from 125 patients. The number of 
cells harvested per patient (over all time points) ranges from 4 to 1415. We
see that for most patients the number of harvested cells ranges between 100 and
500. 

```{r}
length(table(metadata$donor)) # number of donors
range(table(metadata$donor)) # cells per donor
hist(table(metadata$donor),breaks=100) # cells per donor
```

Below, we look how many cells are harvest per patent and per time point (for
the first 25 patients).

```{r}
head(table(metadata$donor,metadata$day),n=25)
```

We see that for many patients the data is complete, i.e. cells were sampled
on all time points.

Practically, the cells were prepared in 28 batches;

```{r}
length(table(metadata$experiment))
table(metadata$experiment, metadata$day)
```
...
(sce <- SingleCellExperiment(assays = list(counts = as.matrix(data[,2:ncol(data)])),
                             colData = metadata,
                             rowData = data[,1]))
```
