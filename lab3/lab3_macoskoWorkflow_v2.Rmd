...
predicted as bipolar cells, and so on). Before we delve deeper into this, we
...
function then allow you to plot, for each cell (columns) the assignment score 
...
this cell was labeled as a rod photoreceptor (top row). This is because this
was the label that was assigned to the cell with highest assignment score 
(third row). We also see that the second most likely label for this cell is
cones (fourth row).
...
We will do the interpretation of the concordance between the predicted and the
...
in principle need to double-check with markers if this is a true biological 
...
**Altogether, label-transfer was quite successfull, and we would have been able**
