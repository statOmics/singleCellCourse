...
**second step, the `cluster_walktrap` function, is slow for large datasets.**
...
applied to a reduced dimensional representation of the expression data (most 
often PCA, because of the interpretability of the low-dimensional distances). 
...
choice of the number of clusters `k` can be guided by known biology, however, 
it is arbitrary at least to some interval.
...
to noise, we may perform this on the top (30) PC's. Calculating a dissimilarity
matrix is implemented in the `dist` function.

2. Performs a hierarchical cluster analysis on the distances from 
step 1. In an agglomerative procedure, each cell is first assigned to its own 
cluster and then the algorithm proceeds iteratively, at each stage joining the 
two most similar clusters, continuing until there is just a single cluster. 
Implemented in the `hclust` function.
...
Next the authors continue with two additional steps, which go beyond the scope
of this session:
...
library(snifter)
embedding <- reducedDim(sce[,sce$detected>900], type="PCA")
