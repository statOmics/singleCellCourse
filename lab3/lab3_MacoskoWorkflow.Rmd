...
1. Import the Macosko dataset as SingleCellExperiment object from the scRNAseq 
...
```{r}
sce <- readRDS("/Users/jg/Desktop/sce_after_labsession2.rds") # 147Gb SCE object
```
...
used by default). An edge is drawn between all pairs of observations that share
...
building a graph and detecting clusters in that graph. For this second step, 
...
```
# Slow and not really informative -> leave out?
library(igraph)
igraph::plot.igraph(
...
A direct comparison using a table of the cluster labels.

```{r}
table(cluster_louvain, cluster_leiden2)
```

First, note that Leiden2 clusters 9-14 all contain only 1 cell, which doesn’t make much sense.
...
For the remain clusters, we see that 
...
we may perform this on the top (30) PC’s. Calculating a dissimilarity matrix 
is implemented in the `dist` function.

2. Perform a hierarchical cluster analysis on the distances from step 1. In an 
...
clusters we want to report, or h, the height in the dendrogram at which we wan to cut the tree.
...
see that the correspondence is not great. As demonstration, I make a table and 
a heathmap comparing the low resolution cluster labels from the paper with our 
...
more comparable to theirs, I will here mimic their strategy for feature 
selection and clustering. **However, I will do this approximatively!** I will 
...
1. **Downsampling:** In order to increase the power of unsupervised 
dimensionality reduction techniques for discovering cell types, the authors 
first downsampled the 49,300-cell dataset to extract single-cell libraries where
900 or more genes were detected.

2. **Feature selection:** We first identified the set of genes that was most 
variable across our training set, after controlling for the relationship between
mean expression and variability. To do this, the authors used a manual 
...
density clustering (DBSCAN algorith) that was not discussed in this lecture 
series. Because this 5th step uses techniques beyond the scope of this course, 
we will simply continue working with the downsampled dataset and perform 
hierarchical clustering. However, I included some code that allows you do 
something similar to what the authors did for your reference (not that running 
...
Binary marker coloring

```{r}
marker_counts_binary <- marker_counts
marker_counts_binary[which(marker_counts_binary > 0)] <- 1
for (i in seq_along(markers)) {
    gg <- ggplot(data = gg_hlp_data,
                    aes(x = x, y = y)) +
        geom_point(aes(color = as.factor(marker_counts_binary[,i])), size = 0.4) +
...
Process reference data (1): Remove unlabeled cells
...
Process reference data (2): remove doublets/contaminant cells
...
Process reference data (3): make lower resolution cell type levels (for easier 
interpretation)
...
Process reference data (4): remove lowly expressed genes
...
Process reference data (5): obtain same gene ID format as in target data
...
Process reference data (6): compute lognormcounts
...
Use SingleR at low reference resolution:
...
Do the same but at high resolution of the reference label
