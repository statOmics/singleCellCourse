...
```{r}
distsce <- dist(reducedDim(sce, "PoiPCA"))
...
plot(reducedDims(sce)$PoiPCA, col = plotcol, pch=16, asp = 1)
lines(SlingshotDataSet(sce), lwd=2, col='black')
```

```{r}
plot(reducedDims(sce)$PoiPCA[,c(1,2)], 
...
sce_fit <- fitGAM(counts = assays(sce)$counts, 
...
for (i in 1:10) {
  sigGeneStart <- oStart[i] # top10 most significant genes
