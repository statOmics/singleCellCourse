...
**I think we best do this on technical repeats, no? So maybe we can "remove"**
...
cs[order(cs, decreasing = TRUE)][c(1,10,50,100,200)]
```

```{r}
targetCells <- names(cs[order(cs, decreasing = TRUE)][c(1,10,50,100,200)])

curCells_list <- list(c(targetCells[1], targetCells[2]),
                      c(targetCells[1], targetCells[3]),
                      c(targetCells[1], targetCells[4]),
                      c(targetCells[1], targetCells[5]))

for (i in seq_along(curCells_list)) {
  curCells <- curCells_list[[i]]
  M <- rowMeans(assays(sce)$counts[,curCells])
  D <- assays(sce)$counts[,curCells[2]] / assays(sce)$counts[,curCells[1]]
  plot(x = log(M), y = log2(D),
       pch = 16, cex=1/3,
       main = paste0("Cell ", curCells[2], " vs cell ", curCells[1]),
...
for (i in seq_along(curCells_list)) {
  curCells <- curCells_list[[i]]
  M <- rowMeans(assays(sce)$normed[,curCells])
  D <- assays(sce)$normed[,curCells[2]] / assays(sce)$normed[,curCells[1]]
  plot(x = log(M), y = log2(D),
       pch = 16, cex=1/3,
       main = paste0("Cell ", curCells[2], " vs cell ", curCells[1]),
       xlab = "Log mean", ylab = "Log2 fold-change",
...
**Comparison 1 looks good to me, but the rest looks like overcorrection, no?**
...
speed of the algorithm for large dataset by applying a heuristic.
