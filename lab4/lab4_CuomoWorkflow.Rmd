...
from 125 healthy human donors, used to study the endoderm 
...
We here show the t-SNE visualizations comparing non-corrected versus the batch
...
We see that most cells of day0 are in hierarchical cluster 3 (arbitrary 
indicator), day1 cells are in cluster 4, day2 cells are mainly in cluster 1 and
fay3 cells are found back in cluster 2.
...
`tradeSeq` is a flexible that tools that allows for studying differential gene
expression along the developmental trajectory, or between different lineages.
...
To assess if all genes could modeled;
...
The `associationTest` function allows for performing a global test that assesses
if the average expression of a gene is associated with pseudotime.
...
We observe a huge number of statistically significant findings. With such many
data points (3.731), it is difficult to control the type I error at the nominal 
level. To prioritize for genes that are also biologically relevant, we may test
against a log-fold change cut-off:

```{r}
assoRes2 <- associationTest(sce_fit, l2fc=1)
...
```{r}
startRes <- startVsEndTest(sce_fit, l2fc=1)
```

We can then visualize the gene expression profile along pseudotime of the top5
...
We also inspect the results in our differential testin output.

Association test:

```{r}
assoRes2[markers,]
# @Koen: third gene is NA, maybe due to too large estimate? fitGAM did converge though...
```
...
```{r}
startRes[markers,]
# @Koen: ideally, ENSG00000164458 should not be DE between start and end.
```

Another interesting visualization is implemented in the `plotGeneCount` 
function, which colors the cells based on the log-normalized expression
