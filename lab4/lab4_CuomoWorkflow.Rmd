...
2. Perform a hierarchical clustering analysis on these distances. Initially, each cell is assigned to its own cluster and then the 
algorithm proceeds iteratively, at each stage joining the two most similar 
clusters, continuing until there is just a single cluster. This is implemented in the
`hclust` function.
...
Mapping clusters to timepoints, we note that most cells of day0 are in hierarchical cluster 3 (arbitrary 
...
The `associationTest` assesses whether the average expression of a gene is associated with pseudotime. To prioritize for genes that are also biologically relevant, we may test
